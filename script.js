...
const modelFragmentShader = `
  ...
  void main(){
    ...
    float fill=max(dot(normal,normalize(vec3(-.6,-.2,.8))),0.0)*.22;
    float back=max(dot(normal,normalize(vec3(.2,-.4,-1.))),0.0)*.14;
    float spec=pow(max(dot(normal,normalize(L+V)),0.0),36.0)*.55;
    float fres=pow(1.0-max(dot(normal,V),0.0),2.6)*.32;
    float grain=(hash(floor(vec3(dot(normal,vec3(31.,7.),dot(normal,vec3(17.,53.)),length(vPosition)*23.))*64.))-.5)*.07;
    vec3 color=diff*uBrightness+fill+back+spec+fres+normal*uNormalStrength+.5+grain;
    color=clamp(color,.12,.995)*uRemapColor;gl_FragColor=vec4(color,1.0);
  }
`;
